...
const signDictionary = {
  ...
  phrases: {
    ...
    'كيف حالك': { name: 'كيف حالك', video: '/sgin_letters/كيف حالك.mp4' },
    'عامل ايه': { name: 'عامل إيه', video: '/sgin_letters/عامل ايه.mp4' },
    'انت كويس': { name: 'إنت كويس', video: '/sgin_letters/انت كويس.mp4' },
    'استنى': { name: 'استنى', video: '/sgin_letters/استنى.mp4' }
  }
};
...
function translate(text) {
  ...
  if (signDictionary.phrases[normalized]) {
    signs.push({
      type: 'phrase',
      value: normalized,
      ...signDictionary.phrases[normalized]
    });
  ...
  for (const word of words) {
    // Check if word is a known phrase
    if (signDictionary.phrases[word]) {
      signs.push({
        type: 'phrase',
        value: word,
        ...signDictionary.phrases[word]
      });
    } else {
      // Split into letters
  ...
  return { 
    signs, 
    mode: signs.some(s => s.type === 'phrase') ? 'mixed' : 'letters' 
  };
}
...
app.get('/', (req, res) => {
  res.json({ 
    ...
    service: 'ESHARA API',
    version: '1.0.0',
    endpoints: ['/api/translate', '/api/dictionary', '/api/health']
  });
});
...
app.post('/api/translate', (req, res) => {
  try {
    const { text, split_unknown_words = true } = req.body;
...
// Speech to Text (mock - uses Web Speech API on frontend)
app.post('/api/speech-to-text', (req, res) => {
  res.json({
    text: '',
    language: 'ar',
    confidence: 0,
    message: 'Use Web Speech API on frontend for speech recognition'
  });
});
...
app.get('/api/dictionary', (req, res) => {
  res.json({
    letters: Object.entries(signDictionary.letters).map(([key, value]) => ({
      letter: key,
      ...value
    })),
    phrases: Object.entries(signDictionary.phrases).map(([key, value]) => ({
      word: key,
      ...value
    })),
...
// Get all letters
app.get('/api/dictionary/letters', (req, res) => {
  res.json(Object.entries(signDictionary.letters).map(([key, value]) => ({
    letter: key,
    ...value
  })));
});
...
// Get all phrases
app.get('/api/dictionary/phrases', (req, res) => {
  res.json(Object.entries(signDictionary.phrases).map(([key, value]) => ({
    word: key,
    ...value
  })));
});
...
app.listen(PORT, () => {
  console.log(`ESHARA API running on port ${PORT}`);
});
